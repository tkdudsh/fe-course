...
형식 > ALTER TABLE [테이블명]
		ADD COLUMN[NEW COLUMN, 데이터 타입] 
        MODIFY COLUMN[MODIFY COLUMN, 데이터 타입]
...
-- phone 컬럼의 크기를 20으로 변경, 크기가 크게 변경하는 경우 정상실행.
alter table emp modify column phone char(20);
desc emp;

-- ename 컬럼의 크기를 varchar(2)로 변경
-- 데이터 유실이 발생하므로, 에러 발생. 데이터 크기가 공간보다 큼.
select * from emp;
alter table emp modify column ename varchar(2) not null;
...
update emp set phone = '010-2250-3274' where eid='S001';
            
            
-- 모든 사원의 폰번호를 '010-1111-1234'
update emp set phone = '010-2250-3274';
select * from emp;
...
alter table emp modify email varchar(25) not null;
desc emp;
